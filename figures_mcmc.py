...
def main():
    ...
    for ifile in np.arange(0,numfiles):
        ...
        #ax.set_yticks(np.arange(0, round(np.max(counts), -1), 2), minor=True)
        plt.ylim((0, round(np.max(counts), -1)))
        plt.xticks(np.linspace(0, math.ceil(np.max(ages)), 5))
        #ax.set_xticks(np.linspace(0, math.ceil(np.max(ages)), 10), minor=True)
        ...
        # get log likelihood for mean of all models within clump

        np.savetxt(args.plotdir+'figures/'+newmcmc.modelName+'_'+str(newmcmc.maxSteps)+'.txt', np.nanmean(newmcmc.logprob).reshape(1))
