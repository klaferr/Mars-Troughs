...
def main():
    ...
    p.add_argument("-trslice", default=1, type=int, help="slice number")
    ...
    if args.retreat == "fit":
        ...
        #create lag model
        retreat_times, retreats, lags = load_retreat_data(trslice)
        retreat_times = -retreat_times
        ret_data_spline = RBS(lag, retreat_times, retreats)
    
    # Create trough object
    xdata, ydata = load_TMP_data(trslice, tmp)
    tr = mt.Trough(acc_model, retr_model, 0, angle)
    mcmcobj = mt.MCMC(xdata, ydata, tr, maxSteps, thin_by, directory)
...
def mainArgs(acc,lag,retr,steps,thin_by,data,tmp,trslice, dir):
